...
CREATE INDEX idx_candidates_clan_batch ON candidates(clan_id, batch);
...
CREATE INDEX idx_votes_clan ON votes(clan_id);
...
-- Admin credentials table (for offline mode password protection)
...
-- RLS Policies: Allow public read access (kiosk mode)
...
-- Insert default admin password (hash of 'admin123' - should be changed)
INSERT INTO admin_credentials (id, password_hash)
VALUES (1, '$2a$10$rGHQcXXXExample.HashHere');
